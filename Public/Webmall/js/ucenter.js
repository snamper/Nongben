...
function showuserLoginInfo() {
    ...
    $.ajax({
        url: baselocation + "/user/loginuser.json",
        data: {},
        type: "post",
        dataType: "json",
        cache: false,
    ...
    } else if (imgUrl != 'notimg' && bannerUrl != '' && typeof(bannerUrl) != 'undefined') {
...
function dialog(dTitle, coent, index, url) {
    ...
    $(".bg-shadow").remove();
    var oBg = $('<div class="bg-shadow"></div>').appendTo($("body")), dialogEle = $('<div class="dialog-shadow"><div class="dialog-ele"><h4 class="d-s-head pr"><a href="javascript:void(0)" title="关闭" class="dClose icon16 pa">&nbsp;</a><span class="d-s-head-txt">' + dTitle + '</span></h4><div id="dcWrap" class="pt20 pb20 pl20 pr20 of" style=""></div></div></div>').appendTo($("body")).addClass("animate-enter animated flyIn5");
    $.ajax({
...
function applicationImg(reqType) {
    ...
    $.ajax({
        ...
        url: baselocation + '/uc/updateusercover.json',
        success: function (result) {
...
function getUserPersonalityImages(pageCount) {
    ...
    $.ajax({
        ...
        url: baselocation + '/uc/usercover.json',
        success: function (result) {
            if (result.success) {
                var obj = result.entity;
...
function openActCouWin(contractid) {
    ...
    $(".cTipsClose").click(function () {
        $(".maskWrap,.maskBg").removeClass("animate-enter animated flyIn5").remove();
    });
};
...
var leftUrlAndClassArr = [
    ...
    'leftdl_course@/uc/favoriteTeacher',
    'leftdl_course@/uc/note',
    'leftdl_course@/uc/home',
    
    'leftdl_course@/uc/mylive',
    
    'leftdl_line_course@/uc/lineCourse',
    'leftdl_line_course@/uc/myLineCouAnswer',
    'leftdl_line_course@/uc/myLineEnroll',

    'leftdl_book@/uc/bookList',
    ...
    'leftdl_account@/uc/userEmailMobile',
    'leftdl_account@/uc/avatar',
    'leftdl_account@/letter',
    'leftdl_account@/uc/myProfile',
    'leftdl_account@/uc/userResume',
    'leftdl_account@/uc/shareFriend'
];
...
function ckeckUrl(){
	...
	var ctx=$("#ctx").val();
	var pathName=ctx+window.location.pathname;
	var urls=$(".u-menu-list>dl>dd a");
	$(".u-menu-list>dl>dd a").removeClass("c-blue");
	for(var i=0;i<urls.length;i++){
		var urlName=urls.eq(i).attr("href");
		if(urlName==pathName){
			urls.eq(i).addClass("c-blue");
		}
            ...
            success: function (result){
                if(result.success == false){
                    if (result.message == "OthersIsLogin") {
                        dialog("下线提示", "您的账号在其他地方登录，若非本人操作，请重新登录。", 6, '/index.json');
                        DeleteCookie("stime");
                    }
                }else{
...
//选项卡切换
function tabToggle(aelm, type) {// 获得免费课程
	if ($(aelm).parent('li').hasClass('current') == false) {
		$(".indexTab").css("display","none");

		$("#goChoose").attr("href","/front/showcoulist.json");
		$("#goChoose").html("去选课");
		if (type == 1) {
			//免费课程
			$(aelm).parent('li').addClass('current');
			$(aelm).parent('li').siblings().removeClass('current');
			...
			$(aelm).parent('li').addClass('current');
			$(aelm).parent('li').siblings().removeClass('current');
			$("#myBuyLive").css('display', 'block');
		}else if(type==4){
			//我购买的图书
			$(aelm).parent('li').addClass('current');
			$(aelm).parent('li').siblings().removeClass('current');
			$("#myBuyBook").css('display', 'block');
			$("#goChoose").attr("href","/book/booklist.json");
			$("#goChoose").html("去选书");
		}else if(type==5) {
			//我购买的套餐
			$(aelm).parent('li').addClass('current');
			$(aelm).parent('li').siblings().removeClass('current');
			$("#myBuyPackCourse").css('display', 'block');
		}
	}
}
...
function allselect(en) {
	 if(en.checked){
		$("input[name='sellIdArr']").prop("checked", true);
	} else {
		$("input[name='sellIdArr']").prop("checked", false);
	}
}
function expire() {
	dialog("课程提示", '课程已过期!', 1);
...
function watchCourse(courseId) {
	location.href = baselocation + '/Webmall/Academy/play?id=' + courseId;
}
...
function stepFun() {
	$
			.ajax({
				url : baselocation + "/user/queryUserGuideStatus.json",
				type : "post",
				async : false,
...
function selectRondomTeacher() {
			$.ajax({
				url : baselocation + "/uc/rantech.json",
				type : "post",
				dataType : "json",
				data : {
...
//选择直播状态
function chooseLive(){
	$(".clearfix.pl20 li").click(function(){
		$(".clearfix.pl20 li").removeClass("current");
		$(this).addClass("current");
		var id=$(this).attr("long");
		var id="#"+id;
		$(".liveStatus").hide();
		$(id).show();
	});
