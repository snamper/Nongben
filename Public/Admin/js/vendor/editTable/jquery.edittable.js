...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        // Build cell
        function buildCell(content, type, index, name) {
            content = (content === 0) ? "0" : (content || '');
            // Custom type
            if (type && 'text' !== type){
                var field = s.field_templates[type];
                var valueObj = field.setValue(field.html, content, index, name);
                if(valueObj.size()>1){
                    var html = '';
                    //console.log(valueObj[1].outerHTML);
                    for(var size=0; size<valueObj.size(); size++){
                        //console.log('size'+ size);
                        html += valueObj[size].outerHTML;
                    }
                    //return '<td>' + valueObj[0].outerHTML + '</td>';
                    //console.log(html);
                    return '<td>' + html + '</td>';
                } else {
                    return '<td>' + valueObj[0].outerHTML + '</td>';
            ...
            // Default
            return '<td><input type="text" name="'+ name +'[]" value="' + content.toString().replace(/"/g, "&quot;") + '" /></td>';
        }

        // Build row
        function buildRow(data, len, index=0) {
            ...
            if (!s.row_template) {
                ...
                for (b = 0; b < (len || data.length); b += 1) {
                    rowcontent += buildCell(data[b], null, index,s.name_template[b]);
                }
            } else {
                // With row template
                for (b = 0; b < s.row_template.length; b += 1) {
                    // For each field in the row
                    rowcontent += buildCell(data[b], s.row_template[b], index, s.name_template[b]);
                }
            }
            var edithtml='';

            if(s.addCol){
                edithtml += '<a class="addrow icon-button" href="#">+</a>';
            }
            if(s.delCol){
                edithtml += '<a class="delrow icon-button" href="#">-</a>';
            }

            if(edithtml != null){
                return $('<tr/>', {
                    html: rowcontent + '<td>'+ edithtml +'</td>'
                });
            } else {
                return $('<tr/>', {
                    html: rowcontent
        ...
        function fillTableData(data) {
            ...
            if (s.headerCols || s.row_template) {
                ...
                for (a = 0; a < crow; a += 1) {
                    buildRow(data[a], '', a).appendTo($table.find('tbody'));
                }
