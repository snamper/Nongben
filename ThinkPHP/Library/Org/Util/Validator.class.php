...
class Validator
{
    ...
    public function __construct($data, $fields = array())
    {
        ...
        $this->_fields = !empty($fields) ? array_intersect_key($data, array_flip($fields)) : $data;
         ...
         $langMessages = array(
                                                    ...
                                                    'email'         => "无效邮箱地址",
                                                    'location'      => "不合法的经纬度数据",
                                                    'lengthBetween' => '长度不合法 大于%d位，小于%d位',
                                                    'mobile'        => "无效的手机号码",
                                                    'url'           => "无效的URL",
                                                    'urlActive'     => "必须是可用的域名",
                                                    'alpha'         => "只能包括英文字母(a-z)",
    ...
    protected function stringLength($value)
    {
        if (function_exists('mb_strlen')) {
            return (strlen($value) + mb_strlen($value,'utf-8')) / 2;
        }
        return strlen($value);
    }
    ...
    /**
     * Validate that a field is a valid mobile
     *
     * @param string $field
     * @param mixed $value
     * @return bool
     */
    protected function validateMobile($field, $value)
    {
        return preg_match("/1\d{10}$/", $value);
    }

    /**
     * Validate that a field is a valid Location
     *
     * @param string $field
     * @param mixed $value
     * @return bool
     */
    protected function validateLocation($field, $value)
    {
    ...
    public function validate()
    {
        foreach ($this->_validations as $v) {
            foreach ($v['fields'] as $field) {
                ...
                $value = isset($data) && !is_null($data) ? $data : null;

                // Don't validate if the field is not required and the value is empty
                if ($v['rule'] !== 'required' && !$this->hasRule('required', $field_name) && (! isset($value) || $value === '')) {
                    continue;
                }
                ...
                $result = call_user_func($callback, $field_name, $value, $v['params']);
                if (!$result) {
                    $this->error($field_name, $v['message'], $v['params']);
                }
            }
        }
